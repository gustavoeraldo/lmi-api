...
class MeasurementsController(Measurements):
    ...
    def create_measurements(
        self, db: Session, measure_in: MeasurementsBase
    ) -> MeasurementsInDB:
        ...
        try:
            ...
            raise GlobalException(
                400, f"Something went wrong trying to insert the data!\nError:{e}"
            )

        return db_measurement

    async def get_test_measures(
        self, db: Session, filters: dict, pagination: Pagination
    ):
        measurements = db.query(Measurements)
        measurements = (
            dynamic_filters(Measurements, measurements, filters)
    ...
    def get_measurements(
        self, db: Session, user_id: int, filters: MeasurementsFilters
    ) -> List[MeasurementsInDB]:
        if filters.start_date != filters.end_date:
            return self.get_measurements_by_date(db, user_id, filters)

        return (
            db.query(Measurements)
            .join(MeasuresType, Measurements.type_id == MeasuresType.m_type_id)
            .filter(
    ...
    def get_measurement(self, db: Session, user_id: int, measurement_id: int):
        return (
            ...
            .filter(
                Measurements.user_id == user_id,
                Measurements.measure_id == measurement_id,
            )
            .first()
        )
    ...
    def delete_measurements(self, db: Session, measurement_id: int, user_id: int):
        ...
        if not self.get_measurement(db, user_id, measurement_id):
            raise GlobalException(
                status_code=status.HTTP_404_NOT_FOUND, message="Measurement not found!"
            )

        db.query(Measurements).filter(
            Measurements.user_id == user_id, Measurements.measure_id == measurement_id
        ).delete()
        db.commit()
        return True
