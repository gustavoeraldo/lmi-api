...
async def get_all_measurements(
    *,
    ...
    measures = await MeasuresController.get_test_measures(db, filters, pagination)
    return measures
...
async def remove_measurement(
    user_id: int, measurement_id: int, db: Session = Depends(get_db)
):
    MeasuresController.delete_measurements(
        db=db, measurement_id=measurement_id, user_id=user_id
    )
    return JSONResponse(
        status_code=status.HTTP_200_OK, content="Measurement was deleted."
    )
